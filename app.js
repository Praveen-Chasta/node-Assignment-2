...
const tweetAccessVerification = async (request, response, next) => {
  ...
  const getTweetQuery = `SELECT * FROM tweet INNER JOIN follower ON tweet.user_id = follower.following_user_id WHERE tweet.tweet_id = '${tweetId}' AND following_user_id = ''${userId}`;
...
app.post("/register/", async (request, response) => {
  ...
  if (userDBDetails !== undefined) {
    ...
    if (password.length < 6) {
      ...
      const createUserQuery = `INSERT INTO user(username,password,name,gender) VALUE ('${username}', '${hashedPassword}', '${name}', '${gender}')`;
...
app.get("/user/followers/", authToken, async (request, response) => {
  ...
  SELECT DISTINCT name FROM follower INNER JOIN user ON user.user_id = follower.following_user_id WHERE follower_user_id = '${userId}'
  `;
...
app.get(
  ...
  async (request, response) => {
    const { tweetId } = request.params;
    const getReplyQuery = `
    SELECT name,reply FROM user INNER JOIN reply ON user.user_id = like.user_id WHERE tweet_id = '${tweetId}'
    `;
